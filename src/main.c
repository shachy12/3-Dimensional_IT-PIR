...
double test_2_servers_pir(pir_server_t *server) {
    ...
    i3 = rand() % server->query_size;

    //create queries
    uint8_t *q1, *q2, *q3;
    __m256i *results_0, *results_1;
    create_queries(server->query_size, &q1, &q2, &q3);
    ...
    double elapsed = 0;
    clock_t start = clock();
    results_0 = pir_answer(server, q1, q2, q3); // server 1
    clock_t end = clock();
    ...
    q2[i2] ^= 1;
    q3[i3] ^= 1;
    results_1 = pir_answer(server, q1, q2, q3); // server 2
    end = clock();

    elapsed += (double)(end - start) / CLOCKS_PER_SEC;
    double throughput = server->db_size_bits / (8 * 1024.0 * 1024.0) / (elapsed / 2);
    printf("Elapsed time: %.6f seconds\n", elapsed);
    printf("throughput per server: %.2f MB/s\n",  throughput);
    
    // reconstruct entry and verify data
    __m256i *entry = reconstruct(i1, i2, i3, results_0, results_1, server->query_size * 3 + 1, server->blocks_per_entry);
    size_t db_index = (i1 * server->query_size * server->query_size + i2 * server->query_size + i3) * (BLOCK_SIZE * server->blocks_per_entry);
...
int main(int argc, char *argv[]) {
    ...
    printf("Average throughput for %d samples: %.2f MB/s, Std: %.2f MB/s\n", samples, avg, std);
    free(db);
    return 0;
}